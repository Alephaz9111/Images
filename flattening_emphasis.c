...
void flattening(unsigned char img[HEIGHT][WIDTH], unsigned char img_flattening[HEIGHT][WIDTH]){ 
    ...
    double flat_emphasis_numerator[9] = {1, 1, 1, 
                                         1, 2, 1,
                                         1, 1, 1};
                               
    double flat_emphasis_denominator[9] = {10, 10, 10, 
                                           10, 10, 10,
                                           10, 10, 10};
    ...
    for( k = 1; k < HEIGHT - 1; k++ ){ 
        for( l = 1; l < WIDTH - 1; l++ ){ 
            ...
            for( i = 0; i < 3; i++ ){
                for( j = 0; j < 3; j++ ){
                    sum += flat_emphasis_numerator[ i * 3 + j ] * img[ k - 1 + i ][ l - 1 + j ] / flat_emphasis_denominator[ i * 3 + j ]; 
                }
            }
            if( sum < 0 ) sum = 0; 
